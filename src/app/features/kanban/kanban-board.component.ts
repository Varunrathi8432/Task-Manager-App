...
export class KanbanBoardComponent {
  ...
  onDrop(event: CdkDragDrop<Task[]>, targetStatus: TaskStatus): void {
    ...
    if (event.previousContainer === event.container) {
      ...
      this.taskStore.moveTask({ id: task.id, status: targetStatus, order: event.currentIndex });
    } else {
      this.taskStore.moveTask({ id: task.id, status: targetStatus, order: event.currentIndex });
    }
  }
  ...
  isOverdue(task: Task): boolean {
    return task.status !== 'done' && !!task.dueDate && isBefore(parseISO(task.dueDate), new Date());
  }
